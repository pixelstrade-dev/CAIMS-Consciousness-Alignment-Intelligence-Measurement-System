...
| Corpus size picked by round number | Seeded power analysis from Run 001's measured parameters: corpus v1 ≈ 200–300 items, 25–30 % adversarial, n=5 suffices for ≥5-pt bound violations, ~100 all-judge items pin judge drift to ±1.7 pts; 3-rater sizing explicitly deferred to real Run 002 data | A6, #62 |
...
published anchors; overclaim hunts). Findings that survive in the record
include two the review turned against this project's own drafts: the A5
...
- The statistics in the repo recompute against published anchors, and
...
1. **Run 002 with ≥ 3 judge families** (owner: add the third provider's
   key as a repository **Secret**, then run the Experiment workflow).
   Unlocks: real 3-rater α/ICC, PIGA classification agreement (both
   channels), 3-rater corpus re-sizing.
